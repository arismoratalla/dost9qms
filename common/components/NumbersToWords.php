 ...
 */
 class NumbersToWords{
    ...
    public static function convert($GivenNumber){
      if (!is_numeric($GivenNumber) ) return false;
      $string = '';
      //Remove Fraction from numbers
      $number=floor($GivenNumber);
      $fraction = $GivenNumber - $number;
      //Make Fraction Whole Number
      $FractionInWhole=$fraction * 100;
      switch (true) {
        case $number < 21:
        ...
        case $number < 1000:
            ...
            if ($remainder) {
                ...
                //$string .= self::$conjunction . $remainder . '/100';
            }
            break;
        default:
      ...
      if($FractionInWhole>0){
         $string .= ' '. self::$conjunction . number_format($FractionInWhole,0) . '/100';
      }
